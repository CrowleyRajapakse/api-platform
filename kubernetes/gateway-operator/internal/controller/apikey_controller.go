...
const apiKeyFinalizer = "gateway.api-platform.wso2.com/apikey-finalizer"
...
func (r *ApiKeyReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&corev1.Secret{},
			handler.EnqueueRequestsFromMapFunc(enqueueAllOfKind(r.Client, &apiv1.ApiKeyList{})),
			builder.WithPredicates(apiKeySecretWatchPredicate(r.Client))).
		Complete(r)
}

func apiKeySecretWatchPredicate(c client.Client) predicate.Funcs {
	return predicate.Funcs{
		CreateFunc: func(e event.CreateEvent) bool {
			...
			return referencedByAnyApiKey(c, s)
		},
		UpdateFunc: func(e event.UpdateEvent) bool {
			s, ok := e.ObjectNew.(*corev1.Secret)
			if !ok {
				return false
			}
			return referencedByAnyApiKey(c, s)
...
func referencedByAnyApiKey(c client.Client, s *corev1.Secret) bool {
	if s == nil {
		return false
	}

	// Reconcile only if any ApiKey in the same namespace references this Secret
	// via spec.apiKey.valueFrom.name.
	var list apiv1.ApiKeyList
	if err := c.List(context.Background(), &list, client.InNamespace(s.Namespace)); err != nil {
		return false
	}
