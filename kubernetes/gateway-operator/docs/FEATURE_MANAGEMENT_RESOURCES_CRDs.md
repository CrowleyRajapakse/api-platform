...
`LlmProviderTemplate`, `LlmProvider`, `LlmProxy`, `Mcp`, `WebSubApi`, `Secret`, `Certificate`, `ApiKey`, `SubscriptionPlan`, `Subscription`
...
| WebSubApi | `/websub-apis/{name}` |
| Secret | `/secrets/{name}` |
| Certificate | `/certificates/{id}` (certificate upload shape differs from apiVersion/kind envelopes) |
| ApiKey | `/<parent>/<parentName>/api-keys/<name>` (parents: rest-apis, llm-providers, llm-proxies, websub-apis) |
...
| `websubapi_types.go` | WebSub API CRD |
...
Add samples for kinds missing them (e.g. secret, certificate, subscription-plan, subscription, api-key, websub-api); existing **`llm-*.yaml`**, **`mcp-proxy.yaml`** should align once CRDs land.
